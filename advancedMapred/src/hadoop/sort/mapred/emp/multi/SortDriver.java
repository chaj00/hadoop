...
public class SortDriver extends Configured implements Tool{
	...
	public int run(String[] optionlist) throws Exception {
		...
		job.setMapOutputValueClass(EmpDTO.class);
		...
		job.setOutputValueClass(EmpDTO.class);

		FileInputFormat.addInputPath(job, new Path(otherArgs[0]));
		FileOutputFormat.setOutputPath(job, new Path(otherArgs[1]));
		//output파일을 세 개 생성할 것이므로 MultipleOutputs설정
		MultipleOutputs.addNamedOutput(job, "insa",TextOutputFormat.class,NullWritable.class, EmpDTO.class);
		MultipleOutputs.addNamedOutput(job, "operation",TextOutputFormat.class,NullWritable.class, EmpDTO.class);
		MultipleOutputs.addNamedOutput(job, "system",TextOutputFormat.class,NullWritable.class, EmpDTO.class);
